...
# Divvy API URL
DIVVY_API_URL = "https://gbfs.divvybikes.com/gbfs/2.3"

class DivvyService(BikeShareService):
    """Service for accessing Divvy (Chicago) bike sharing data."""
    
    def __init__(self):
        super().__init__(DIVVY_API_URL, "divvy")
    
    async def get_stations(self) -> List[StationResponse]:
        """
        Get information for all Divvy stations with pre-calculated
...
divvy_service = DivvyService()
# Export the router for use in the main application
router = divvy_service.router